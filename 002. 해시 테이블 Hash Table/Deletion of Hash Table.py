...
class HashTable:
    ...
    def delete_by_key(self, key):
        ...
        linked_list = self._get_linked_list_for_key(key)
            # 전달받은 key를 해시 함수로 변환하여 배열에 접근하고, 해당 인덱스에 있는 리스트를 linked_list에 저장한다
        to_delete = self._look_up_node(key)
            # 전달받은 key를 활용하여 해당 key를 갖는 노드를 가져온다
        return linked_list.delete(to_delete)
            # LinkedList 클래스의 delete 메소드를 이용하여 해당 노드를 제거한다
