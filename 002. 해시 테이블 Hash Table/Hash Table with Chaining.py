''' 충돌 방지를 위해 Chaining을 사용하는 해시 테이블 구현 '''
...
class HashTable:
    ...
    def _get_linked_list_for_key(self, key):
        ...
        hashed_index = self._hash_function(key)
            # 해시 함수를 통해 변환된 key 값을 저장

        return self._table[hashed_index]
            # 변환된 key 값을 인덱스로 하는 배열에 저장된 링크드 리스트를 반환


    def _look_up_node(self, key):
        """파라미터로 받은 key를 갖고 있는 노드를 리턴하는 메소드"""
        linked_list = self._get_linked_list_for_key(key)
            # _get_linked_list 메소드를 통해 해당 배열에 저장된 링크드 리스트를 받아온 뒤 저장
        return linked_list.find_node_with_key(key)
            # 받아온 리스트에서 key값을 활용하여 탐색을 진행하고, 탐색 실패 시 None을 반환
