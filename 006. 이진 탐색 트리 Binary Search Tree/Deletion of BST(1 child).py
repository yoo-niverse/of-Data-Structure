        ...
        if node_to_delete.left_child is None or node_to_delete.right_child is None:
            if self.root is node_to_delete:
                ...
                if node_to_delete.left_child is None:
                    double_child = node_to_delete.right_child
                        # 왼족 자식이 없다면 오른쪽 자식 노드를 double_child 변수에 저장
                else:
                    double_child = node_to_delete.left_child
                        # 그렇지 않다면 왼쪽 자식 노드를 double_child 변수에 저장

                if double_child.data > parent_node.data:
                        # double_child는 삭제하려는 노드의 자식 노드. 이 값이 삭제할 노드의 부모노드보다 크다면
                    parent_node.right_child = double_child
                    node_to_delete.left_child = None
                        # 삭제할 노드의 부모 노드와 자식 노드를 연결해주고, 삭제할 노드의 자식관계는 제거
                    
                else:
                    parent_node.left_child = double_child
                    node_to_delete.right_child = None
                        # 삭제할 노드의 부모노드보다 작은 값이라면 왼쪽 자식이 되어야한다.

                double_child.parent = parent_node
                    # 조건에 따라 레퍼런스 설정을 진행 한 후 자식 노드의 부모로 삭제할 노드의 부모 노드를 지정
