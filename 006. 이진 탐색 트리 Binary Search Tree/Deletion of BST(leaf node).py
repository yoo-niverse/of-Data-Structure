...
class BinarySearchTree:
    ...
    def delete(self, data):
        ...
        if node_to_delete.left_child is None and node_to_delete.right_child is None:
            # 삭제할 노드의 왼쪽 자식과 오른쪽 자식이 모두 None 이라면 : leaf 노드가 맞음
            if node_to_delete is self.root:
                self.root = None
                # 삭제할 노드가 root 노드라면 : root 노드 자체를 None으로 처리
                
            if node_to_delete.data < parent_node.data:
                parent_node.left_child = None
                # 삭제할 노드의 값이 부모노드의 값보다 작다면 : 부모의 왼쪽 자식이므로 왼쪽 자식 레퍼런스를 None으로 갱신 
                
            else:
                parent_node.right_child = None
                # 삭제할 노드의 값이 부모노드의 값보다 크다면 : 부모의 오른쪽 자식이므로 오른쪽 자식 레퍼런스를 None으로 갱신
