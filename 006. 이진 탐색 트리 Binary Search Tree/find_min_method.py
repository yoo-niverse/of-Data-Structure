''' 주어진 노드를 root로 갖는 트리에서 가장 작은 노드를 리턴해주는 메소드 find_min 구현 '''
...
class BinarySearchTree:
    ...
    def find_min(node):
        ...
        left = node
            # BST의 작은 값은 주어진 노드의 왼쪽에 있다. left 변수를 통해 자식의 자식으로 내려간다.

        while left.left_child is not None:
            left = left.left_child
            # left의 왼쪽 자식이 None이 아닌 동안 반복 수행 : left 변수를 그의 왼쪽 자식으로 갱신한다.

        return left
            # 수행 종료 후 가장 왼쪽에 있는 leaf 노드를 반환
